...
const buildPrompt = (profile: ProfileState, intent: UserIntent) => {
    ...
    switch (intent) {
        ...
        case 'find_event':
            ...
            intentText = `The user wants to find a specific, timed event. Today's date is ${formattedDate}. All suggested events MUST take place in the future, after this date. Prioritize suggestions like concerts, art gallery openings, workshops, food festivals, or live performances happening soon. The title should include the event name and the full date of the event if available.`;
            break;
        default:
...
export const generateImage = async (prompt: string): Promise<string> => {
  try {
    const response = await ai.models.generateImages({
      ...
      prompt: `A vibrant, high-quality, photorealistic image for a dating app suggestion, style of a travel blog photo. Keywords: ${prompt}`,
      config: {
        numberOfImages: 1,
