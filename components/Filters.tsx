...
interface FiltersProps {
  onSearch: (profile: ProfileState) => void;
...
const PremiumLock: React.FC = () => (
    <div className="relative p-6 text-center bg-gray-100 dark:bg-stone-800 rounded-lg overflow-hidden my-4">
...
const Filters: React.FC<FiltersProps> = ({ onSearch, initialProfile, isLoading, userIntent, subscriptionStatus, searchCount }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // The `required` attribute on the input fields handles validation.
    // If this function is called, we know the required fields are filled.
    onSearch(profile);
  };
  
  const hasReachedFreeLimit = subscriptionStatus === 'free' && searchCount >= 5;
  // REMOVED field validation from this check. Button is disabled only when loading or limit is reached.
  // The browser's native `required` field validation is now used instead.
